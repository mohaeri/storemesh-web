...
function printCards(items){return items.length?`<div class="print-list">${items.map(j=>`<article><span class="label-icon">▤</span><div><b>${esc(j.label)}</b><small>${esc(j.entityType)} · تلاش ${j.attempts||0}</small></div>${status(j.status)}<div class="actions">${j.status==='PENDING'?`<button data-print="${j.id}" data-print-action="complete">چاپ شد</button><button data-print="${j.id}" data-print-action="fail">خطا</button>`:j.status==='FAILED'?`<button data-print="${j.id}" data-label="${esc(j.label)}" data-print-action="retry">اسکن و چاپ مجدد</button>`:''}</div></article>`).join('')}</div>`:empty('صف چاپ خالی است')}
...
function bindShell(){
 ...
 document.querySelectorAll('[data-print]').forEach(b=>b.onclick=()=>act(()=>request(`/api/print-jobs/${b.dataset.print}/${b.dataset.printAction}`,{method:'POST',body:b.dataset.printAction==='fail'?JSON.stringify({reason:'خطای چاپگر'}):b.dataset.printAction==='retry'?JSON.stringify({verifiedScan:b.dataset.label}):undefined}), 'وضعیت چاپ به‌روزرسانی شد'));
