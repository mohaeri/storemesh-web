...
const claims=()=>{try{return JSON.parse(atob(state.token.split('.')[0].replace(/-/g,'+').replace(/_/g,'/')))}catch{return{}}};
...
async function ensureSession(){if(!state.sessionId){state.sessionId=(await post('/api/sessions',{operatorId:actor(),station:'web-console'})).id;sessionStorage.sessionId=state.sessionId}return state.sessionId}
...
function renderPage(){
 ...
 case'containers':return `<div class="split-head"><div><h2>مدیریت کانتینرها</h2><p>کنترل ظرفیت، محصول یکتا و جابه‌جایی گروهی</p></div></div>
  <div class="two-col">${panel('ایجاد کانتینر',`<form data-action="container" class="stack-form">${select('نوع','type',['BASKET','BIN','PALLET','TANK'])}${field('ظرفیت kg','capacityKg','number','100','min="1"')}${select('زون اولیه','zone',['RECEIVING','COLD_ROOM','SORTING','PROCESSING'])}<button class="primary">ایجاد و چاپ کد</button></form>`)}
 ...
 case'production':return `<div class="process-strip">${['SORT','WASH','SLICE','FREEZE','FREEZE_DRY','DRY','MERGE'].map((x,i)=>`<div><span>${i+1}</span><b>${x}</b></div>`).join('')}</div>
  <div class="two-col">${panel('اجرای فرآیند تولید',`<form data-action="transform" class="stack-form"><label><span>بچ ورودی</span><select name="batchId">${options(inv.filter(x=>x.weightKg>0))}</select></label>${select('فرآیند','process',['WASH','SLICE','FREEZE','FREEZE_DRY','DRY','MERGE'])}${field('وزن مصرفی kg','consumeWeightKg','number','10','step=".001"')}${field('وزن خروجی kg','outputWeightKg','number','9.2','step=".001"')}<div class="form-row">${field('محصول خروجی','product','','Truffle')}${field('گرید','grade','','A')}</div><button class="primary">ثبت فرآوری و شجره</button></form>`)}
  ${panel('تفکیک چندخروجی',`<form data-action="sorting" class="stack-form"><label><span>بچ ورودی</span><select name="batchId">${options(inv.filter(x=>x.weightKg>0))}</select></label><div class="form-row">${field('خروجی A kg','a','number','5')}${field('خروجی B kg','b','number','3')}</div><button class="primary">اجرای Sorting</button></form>`)}</div>
 ...
 case'packaging':return `<div class="two-col">${panel('ایجاد بسته',`<form data-action="package" class="stack-form">${select('نوع بسته','type',['BOX','BAG','JAR','PALLET'])}<label><span>بچ موجود</span><select name="batchId">${options(inv.filter(x=>x.weightKg>0))}</select></label>${field('وزن بسته kg','weightKg','number','2','step=".001"')}<button class="primary">ایجاد بسته و لیبل</button></form>`)}
  ...
  ${panel('فهرست بسته‌ها',table(['کد','نوع','اقلام','وزن','ارسال'],packages.map(p=>`<tr><td><b>${esc(p.code)}</b></td><td>${esc(p.type)}</td><td>${p.items?.length||0}</td><td>${fa((p.items||[]).reduce((n,x)=>n+x.weightKg,0))} kg</td><td>${p.shipmentId?status('ASSIGNED'):status('READY')}</td></tr>`)))}`;
 case'shipments':return `<div class="two-col">${panel('ایجاد محموله',`<form data-action="shipment" class="stack-form">${select('سایت مقصد','destinationSite',['DUBAI','ROME','IRAN'])}<label><span>بسته‌های آماده</span><select name="packageId">${options(packages.filter(x=>!x.shipmentId))}</select></label><button class="primary">ایجاد Shipment</button></form>`)}${panel('چرخه ارسال','<div class="timeline"><span class="done">READY</span><i></i><span>LOADED</span><i></i><span>DISPATCHED</span><i></i><span>DELIVERED</span></div>')}</div>
...
function printCards(items){return items.length?`<div class="print-list">${items.map(j=>`<article><span class="label-icon">▤</span><div><b>${esc(j.label)}</b><small>${esc(j.type)} · تلاش ${j.attempts||0}</small></div>${status(j.status)}<div class="actions">${j.status==='PENDING'?`<button data-print="${j.id}" data-print-action="complete">چاپ شد</button><button data-print="${j.id}" data-print-action="fail">خطا</button>`:j.status==='FAILED'?`<button data-print="${j.id}" data-print-action="retry">تلاش مجدد</button>`:''}</div></article>`).join('')}</div>`:empty('صف چاپ خالی است')}
...
function bindShell(){
 ...
 document.querySelectorAll('[data-print]').forEach(b=>b.onclick=()=>act(()=>request(`/api/print-jobs/${b.dataset.print}/${b.dataset.printAction}`,{method:'POST',body:b.dataset.printAction==='fail'?JSON.stringify({reason:'خطای چاپگر'}):undefined}), 'وضعیت چاپ به‌روزرسانی شد'));
...
async function handleForm(e){
 e.preventDefault();const f=e.currentTarget,a=f.dataset.action,v=Object.fromEntries(new FormData(f));try{
  let result;
  if(a==='receive')result=await post('/api/receiving',{...v,weightKg:Number(v.weightKg),sessionId:await ensureSession()});
  ...
  if(a==='transform')result=await post('/api/transforms',{process:v.process,inputs:[{batchId:v.batchId,consumeWeightKg:Number(v.consumeWeightKg)}],outputWeightKg:Number(v.outputWeightKg),product:v.product,grade:v.grade,sessionId:await ensureSession()});
  if(a==='sorting')result=await post('/api/sorting',{batchId:v.batchId,outputs:[{grade:'A',size:'Large',weightKg:Number(v.a)},{grade:'B',size:'Mixed',weightKg:Number(v.b)}],sessionId:await ensureSession()});
  ...
  if(a==='package')result=await post('/api/packages',{type:v.type,items:[{batchId:v.batchId,weightKg:Number(v.weightKg)}]});
