...
test('printing page exposes search-driven existing-label reprint with fixed reasons',async()=>{
  ...
  assert.match(page,/reasonCode.*PRINT_REASON_CODES/);for(const reason of['PRINTER_ERROR','PAPER_FINISHED','RIBBON_FINISHED','DAMAGED_LABEL','LOST_LABEL','POOR_PRINT_QUALITY','CUSTOMER_REQUEST','OTHER'])assert.match(source,new RegExp(reason));
